...
Public Class requestApproval
        ...
        If Connect() Then
            Try
                Dim query As String = "SELECT
                                        ...
                                        WHERE
	                                        request_form_cart.approval_status = 'Pending' AND
    ...
    Sub Approve()
        ...
        If approvalTable.SelectedCells.Count > 0 Then
            ...
            itemID = approvalTable.Rows(selectedRowIndex).Cells(item_id).Value?.ToString()
            reqID = approvalTable.Rows(selectedRowIndex).Cells(prColumn).Value?.ToString()

            If Not String.IsNullOrEmpty(textBoxForOfficer.Text) Then
                Dim result As DialogResult = MessageBox.Show($"Proceed to approve ID:{itemID} with Purchase Request ID:{reqID}", "APPROVE", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning)

                If result = DialogResult.OK Then
                    If Connect() Then
                        Try
                            Dim query As String = $"UPDATE request_form_cart SET approval_status = 'Approved', approved_by = '{textBoxForOfficer.Text}' WHERE item_id = '{itemID}'"
    ...
    Sub reject()
        ...
        If approvalTable.SelectedCells.Count > 0 Then
            ...
            If Not String.IsNullOrEmpty(textBoxForOfficer.Text) Then
                Dim result As DialogResult = MessageBox.Show($"Proceed to reject Request ID:{itemID}", "REJECT?", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning)

                If result = DialogResult.OK Then
                    If Connect() Then
                        Try
                            Dim query As String = $"UPDATE request_form_cart SET approval_status = 'Rejected', rejected_by = '{textBoxForOfficer.Text}' WHERE item_id = '{itemID}'"
    ...
    Sub saveData()
        ...
        If Connect() Then
            ...
            Else
                ...
                VALUES ('{reqID}','{textBoxForOfficer.Text}')"
